...
function RouteModal({ onClose, existing }) {
  ...
  const addStopRow = () => setStops(prev => [
    ...prev,
    { stop_name: '', stop_order: prev.length + 1, pickup_time: '', drop_time: '', landmark: '' }
  ])
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Full Name">
              <Input value={form.name}
                ...
                required placeholder="Driver full name" />
            </Field>
            <Field label="Phone">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="License Number">
              <Input value={form.license_no}
                ...
                required placeholder="e.g. MH1220230012345" />
            </Field>
              ...
              {stops?.map(stop => (
                <div key={stop.id}
                  ...
                  <div className="flex-1">
                    ...
                    {stop.landmark && (
                      <span className="text-gray-400 ml-2 text-xs">{stop.landmark}</span>
                    )}
                  </div>
                  {stop.pickup_time && (
                  ...
                  <div>
                    ...
                    {route.vehicle_no && (
                      <p className="text-xs text-blue-600 mt-0.5">
                        🚌 {route.vehicle_no} ({route.vehicle_type}) · 👨‍✈️ {route.driver_name || 'No driver'}
                      </p>
                    )}
                  </div>
                </div>
                  ...
                  <tr key={v.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => setVehicleModal(v)}
                          className="text-xs px-3 py-1 border border-gray-300 rounded-lg hover:bg-gray-50">Edit</button>
                        <button onClick={() => { if (confirm('Delete vehicle?')) deleteVehicleMutation.mutate(v.id) }}
                          className="text-xs px-3 py-1 border border-red-200 text-red-500 rounded-lg hover:bg-red-50">Del</button>
                      </div>
                    </td>
                  </tr>
                ))}
                    ...
                    <td className="px-5 py-3">
                      <div className="flex gap-2">
                        <button onClick={() => setDriverModal(d)}
                          className="text-xs px-3 py-1 border border-gray-300 rounded-lg hover:bg-gray-50">Edit</button>
                        <button onClick={() => { if (confirm('Delete driver?')) deleteDriverMutation.mutate(d.id) }}
                          className="text-xs px-3 py-1 border border-red-200 text-red-500 rounded-lg hover:bg-red-50">Del</button>
                      </div>
                    </td>
                  </tr>
