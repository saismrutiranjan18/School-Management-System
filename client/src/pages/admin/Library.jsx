...
const ICONS = {
  ...
  party:   ['M21.5 12.5c0 4.97-4.03 9-9 9s-9-4.03-9-9 4.03-9 9-9 9 4.03 9 9z','M12 8v4l2.5 2.5'],
  empty:   ['M4 19.5A2.5 2.5 0 0 1 6.5 17H20','M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z'],
}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg">
        <h2 className="text-lg font-semibold mb-4">{isEdit ? 'Edit Book' : 'Add Book'}</h2>
        ...
        <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form) }} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={form.rack_no} onChange={e => setForm({ ...form, rack_no: e.target.value })}
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
        {success ? (
          <div className="text-center py-4">
            ...
            <h2 className="text-lg font-semibold text-gray-800 mb-2">Book Issued!</h2>
            <p className="text-sm text-gray-600 mb-1">
              <strong>{book.title}</strong> issued to <strong>{selectedStudent?.name}</strong>
            </p>
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
        {result ? (
          <div className="text-center py-4">
            ...
            <h2 className="text-lg font-semibold text-gray-800 mb-2">Book Returned!</h2>
            {result.fine_amount > 0 && (
              <div className={`p-3 rounded-lg mb-3 text-sm
                ${result.fine_paid ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700'}`}>
                Fine: ₹{result.fine_amount} — {result.fine_paid ? '✓ Collected' : '⚠ Pending'}
              </div>
            )}
            <button onClick={onClose}
              className="px-6 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">Done</button>
          </div>
        ) : (
          <>
            ...
            <div className="space-y-2 mb-4 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-800">{issue.book_title}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Student</span>
                <span className="font-medium text-gray-800">{issue.student_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Due Date</span>
                <span className={overdue > 0 ? 'text-red-600 font-medium' : 'text-gray-800'}>
              ...
              {overdue > 0 && (
                <>
                  <div className="flex justify-between">
                    ...
                    <span className="text-red-600 font-semibold">{overdue} days</span>
                  </div>
                  <div className="flex justify-between border-t border-gray-100 pt-2">
                    <span className="text-gray-500">Fine Amount</span>
                    <span className="text-red-600 font-bold text-base">₹{fine.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center gap-2 pt-1">
                    <input type="checkbox" id="fine_paid" checked={finePaid}
                      onChange={e => setFinePaid(e.target.checked)}
                      className="w-4 h-4 accent-green-500" />
                    <label htmlFor="fine_paid" className="text-sm text-gray-700">
              ...
              {overdue === 0 && (
                <div className="flex items-center gap-2 p-2 bg-green-50 border border-green-200 rounded-lg text-green-700 text-xs">
                  <Icon d={ICONS.check} size={14} className="text-green-600" />
...
export default function Library() {
  ...
  const summaryStats = [
    { label: 'Total Books',  value: books.reduce((s, b) => s + parseInt(b.total_copies), 0),     color: 'text-gray-800'  },
    { label: 'Available',    value: books.reduce((s, b) => s + parseInt(b.available_copies), 0),  color: 'text-green-600' },
    { label: 'Issued',       value: issuedBooks.length,  color: 'text-blue-600'  },
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage books, issue and return tracking</p>
        </div>
        {tab === 'catalog' && (
        ...
        <>
          <div className="flex gap-3 mb-4 flex-wrap">
            ...
            <label className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer">
              <input type="checkbox" checked={filterAvail}
                  ...
                  {books.length === 0 && (
                    <tr><td colSpan={7} className="text-center py-12">
                      <div className="flex flex-col items-center gap-2 text-gray-300">
                        <Icon d={ICONS.empty} size={40} />
                        <span className="text-gray-400 text-sm">No books found.</span>
                      </div>
                    </td></tr>
                  )}
                  {books.map(book => (
                    <tr key={book.id} className="hover:bg-gray-50">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-10 bg-blue-100 text-blue-600 rounded flex items-center justify-center shrink-0">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`font-semibold ${parseInt(book.available_copies) > 0 ? 'text-green-600' : 'text-red-500'}`}>
                          {book.available_copies}
                        </span>
                      </td>
                      <td className="px-5 py-3 text-gray-400 text-xs">{book.rack_no || '—'}</td>
                      <td className="px-5 py-3">
                        <div className="flex gap-1.5">
                          ...
                          <button onClick={() => setModal(book)}
                            className="text-xs px-2 py-1 border border-gray-300 rounded-lg hover:bg-gray-50">Edit</button>
                          <button onClick={() => { if (confirm('Delete this book?')) deleteMutation.mutate(book.id) }}
                            className="text-xs px-2 py-1 border border-red-200 text-red-500 rounded-lg hover:bg-red-50">Del</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {/* ── ISSUED TAB ── */}
      {tab === 'issued' && (
        issuedLoading ? <p className="text-gray-400 text-sm">Loading…</p> : (
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wide">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                {issuedBooks.length === 0 && (
                  <tr><td colSpan={6} className="text-center py-12">
                    <div className="flex flex-col items-center gap-2 text-gray-300">
                      <Icon d={ICONS.check} size={40} className="text-green-400" />
                      <span className="text-gray-400 text-sm">No books currently issued.</span>
                    </div>
                  </td></tr>
                )}
                {issuedBooks.map(issue => (
                  <tr key={issue.id} className={`hover:bg-gray-50 ${issue.is_overdue ? 'bg-red-50/30' : ''}`}>
                    <td className="px-5 py-3">
                      <p className="font-medium text-gray-800">{issue.book_title}</p>
                      <p className="text-xs text-gray-400">{issue.book_author}</p>
                    </td>
                    <td className="px-5 py-3">
                      <p className="text-gray-800">{issue.student_name}</p>
                      <p className="text-xs text-gray-400">{issue.class_name} {issue.section}</p>
                    </td>
                    <td className="px-5 py-3 text-gray-500">
                      {new Date(issue.issue_date).toLocaleDateString('en-IN')}
                    </td>
                    <td className="px-5 py-3">
                      <span className={issue.is_overdue ? 'text-red-600 font-medium' : 'text-gray-600'}>
                        {new Date(issue.due_date).toLocaleDateString('en-IN')}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
                      {issue.is_overdue ? (
                        <span className="px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-xs font-medium">
                          Overdue — ₹{parseFloat(issue.current_fine).toFixed(2)} fine
                        </span>
                      ) : (
                        <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-xs font-medium">
                          {issue.days_remaining >= 0 ? `${issue.days_remaining}d left` : 'Due today'}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => setReturnModal(issue)}
      ...
      {/* ── OVERDUE TAB ── */}
      {tab === 'overdue' && (
        overdueLoading ? <p className="text-gray-400 text-sm">Loading…</p> : (
          <>
            {overdueData?.count > 0 && (
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                  <p className="text-xs text-red-500">Overdue Books</p>
                  <p className="text-2xl font-bold text-red-600 mt-1">{overdueData.count}</p>
                </div>
                  ...
                  {(!overdueData?.records || overdueData.records.length === 0) && (
                    <tr><td colSpan={7} className="text-center py-12">
                      <div className="flex flex-col items-center gap-2">
                        <Icon d={ICONS.check} size={40} className="text-green-400" />
                        <span className="text-gray-400 text-sm">No overdue books!</span>
                      </div>
                    </td></tr>
                  )}
                  {overdueData?.records?.map(issue => (
                    <tr key={issue.id} className="hover:bg-gray-50">
                      <td className="px-5 py-3 font-medium text-gray-800">{issue.book_title}</td>
                      <td className="px-5 py-3">
                        <p className="text-gray-800">{issue.student_name}</p>
                        <p className="text-xs text-gray-400">{issue.class_name} {issue.section}</p>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center text-red-600 font-bold">{issue.overdue_days}</td>
